...
## 11) Giller Preview 모드
- 미승인 사용자(unapproved)가 Giller 기능을 제한적으로 탐색할 수 있는 프리뷰 모드
  - 사용자 위치 기반 6km 반경 미션 필터링 (`cd59e99`)
  - Earnings 화면 및 하단 Giller 탭 표시 (`2fc19e1`)
  - 실제 수행 불가 액션 인터셉트 + 잠금 아이콘 표시 (`cca7634`, `1acb906`, `0ef95a9`)
  - 길러 신청 전 기능 체험 유도 목적 (`2581a3b`)

## 12) NHN Cloud 알림톡 연동
- App Push 실패/불가 시 Giller에게 NHN Cloud Alimtalk을 fallback으로 발송 (`18cb30b`)
- 푸시 알림 서비스: `src/services/notification-service.ts`

## 13) 보증금 환불 정책 강화
- 픽업 완료 후 취소 시 보증금 전액 공제 (`7f46ed8`)
- 결제 위젯 플레이스홀더 추가
- Admin 웹 deposit 환불 이슈 수정 (`34c7800`)

## 14) 기능 공백(문서/코드 불일치 후보)
...
- B2B/외부 파트너 컬렉션을 관리자에서 어떻게 운영할지(대시보드/권한/정산) — B2B Professional Giller 신청은 `632a50f` 추가 후 `7a2e9b2`에서 revert됨. dynamic B2B pricing logic(`15dd0d2`)은 유지됨
- Giller Preview → 정식 전환 기준/플로우 정의 필요
