...
## Design Context (UI 작업 기준)

> 상세 내용: `.impeccable.md` 참고. 아래는 핵심 요약.

### 디자인 원칙

1. **한 화면, 한 결정** — 핵심 행동 하나에 집중. 부가 정보는 접거나 다음 단계로.
2. **숫자·상태가 주인공** — 금액, 거리, 상태는 크고 굵게 (fontWeight 800). 설명은 작고 회색으로.
3. **컬러는 의미를 담는다** — 틸(`#0F766E`)은 브랜드·강조에만. 장식 목적 컬러 금지.
4. **층위는 배경색으로** — 흰 카드 / `#F4F7F5` 배경 / `#D7F2EC` 민트 강조. 과도한 shadow 지양.
5. **여백이 디자인이다** — 정보를 줄일 수 없으면 여백을 늘린다.
...
토스 (숫자 중심 미니멀), 카카오택시 (지도+카드 레이어), 당근마켓 (친근한 리스트)

### 규칙 변경 이력

| 날짜 | 변경 |
|---|---|
| 2026-04-01 | teach-impeccable로 Design Context 추가. 미니멀·틸·토스/카카오택시/당근 레퍼런스. |
