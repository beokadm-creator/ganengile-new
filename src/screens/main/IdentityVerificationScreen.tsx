...
const PROVIDERS: Array<{ key: VerificationProvider; title: string; subtitle: string; accent: string; accentDisabled: string }> = [
  { key: 'pass', title: 'PASS 본인확인', subtitle: '국내 사용자를 위한 기본 CI 본인확인 경로입니다.', accent: Colors.primary, accentDisabled: Colors.textTertiary },
  { key: 'kakao', title: '카카오 본인확인', subtitle: '보조 공급자 또는 대체 경로로 함께 준비합니다.', accent: Colors.warning, accentDisabled: Colors.textTertiary },
];
...
export default function IdentityVerificationScreen({ navigation }: { navigation: MainStackNavigationProp }) {
  ...
  async function handleApplyResult() {
    ...
    try {
      ...
      if (applied) {
        Alert.alert('본인확인 반영 완료', '이제 길러 전환을 위한 신청 절차로 바로 이어집니다.', [{ text: '계속', onPress: () => navigation.navigate('GillerApply') }]);
        return;
      }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity style={[styles.primaryButton, loading && styles.primaryButtonDisabled]} onPress={() => void handleApplyResult()} disabled={loading}>{loading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.primaryButtonText}>결과 반영하기</Text>}</TouchableOpacity>
    </ScrollView>
  );
}

const styles = StyleSheet.create({ container: { flex: 1, backgroundColor: Colors.background }, content: { padding: 16, paddingBottom: 32, gap: 12 }, centerState: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.background }, centerText: { color: Colors.textSecondary }, hero: { backgroundColor: Colors.surface, borderRadius: 18, padding: 18, gap: 6 }, kicker: { color: Colors.primary, fontSize: Typography.fontSize.sm, fontWeight: Typography.fontWeight.bold, textTransform: 'uppercase', letterSpacing: 1.1 }, title: { color: Colors.textPrimary, fontSize: Typography.fontSize['4xl'], fontWeight: Typography.fontWeight.bold }, subtitle: { color: Colors.textSecondary, fontSize: Typography.fontSize.base, lineHeight: 21 }, modeCard: { borderRadius: 18, padding: 16, gap: 6 }, modeCardTest: { backgroundColor: Colors.warningLight, borderWidth: 1, borderColor: Colors.warning }, modeCardLive: { backgroundColor: Colors.success + '1A', borderWidth: 1, borderColor: Colors.success }, modeTitle: { color: Colors.textPrimary, fontSize: Typography.fontSize.base, fontWeight: Typography.fontWeight.bold }, modeBody: { color: Colors.textSecondary, fontSize: Typography.fontSize.sm, lineHeight: 19 }, modeMeta: { color: Colors.textSecondary, fontSize: Typography.fontSize.sm }, statusCard: { backgroundColor: Colors.surface, borderRadius: 18, padding: 18, gap: 10 }, sectionTitle: { color: Colors.textPrimary, fontSize: Typography.fontSize.xl, fontWeight: Typography.fontWeight.bold }, statusBadge: { alignSelf: 'flex-start', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 6 }, statusBadgeText: { fontWeight: Typography.fontWeight.bold }, statusDescription: { color: Colors.textSecondary, lineHeight: 20 }, inlineButton: { alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: Colors.primaryMint }, inlineButtonText: { color: Colors.primary, fontWeight: Typography.fontWeight.bold }, providerSection: { gap: 10 }, providerCard: { backgroundColor: Colors.surface, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: Colors.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }, providerCardSelected: { borderColor: Colors.primary }, providerCardDisabled: { opacity: 0.6 }, providerCopy: { flex: 1, gap: 4 }, providerTitle: { color: Colors.textPrimary, fontWeight: Typography.fontWeight.bold }, providerSubtitle: { color: Colors.textSecondary, fontSize: Typography.fontSize.sm, lineHeight: 19 }, providerMeta: { color: Colors.textSecondary, fontSize: Typography.fontSize.sm }, providerAction: { fontWeight: Typography.fontWeight.extrabold }, noticeBox: { backgroundColor: Colors.surface, borderRadius: 18, padding: 18, gap: 6 }, noticeTitle: { color: Colors.textPrimary, fontWeight: Typography.fontWeight.bold }, noticeBody: { color: Colors.textSecondary, lineHeight: 20 }, primaryButton: { minHeight: 54, borderRadius: 18, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center' }, primaryButtonDisabled: { opacity: 0.7 }, primaryButtonText: { color: Colors.surface, fontWeight: Typography.fontWeight.extrabold, fontSize: Typography.fontSize.lg } });
